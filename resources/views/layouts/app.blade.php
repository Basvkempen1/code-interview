...
<div x-data="setup()" x-init="$refs.loading.classList.add('hidden'); setColors(color);" :class="{ 'dark': isDark}">
    <div class="flex h-screen antialiased text-gray-900 bg-gray-100 dark:bg-dark dark:text-light">
        ...
        <div
            x-ref="loading"
            class="fixed inset-0 z-50 flex items-center justify-center text-2xl font-semibold text-white bg-primary dark:bg-darker"
        >
            Loading.....
        </div>
        ...
        <div class="flex flex-col flex-1 h-full overflow-x-hidden overflow-y-auto">
            <!-- Navbar -->
            <header class="flex-shrink-0 relative bg-white dark:bg-darker">
                <div class="flex items-center justify-between p-2 px-4 border-b dark:border-primary-darker">
                    <!-- Mobile menu button -->
                    <button
                                ...
                                <x-jet-dropdown align="right" width="60">
                                    <x-slot name="trigger">
                                        <span class="inline-flex rounded-md">
                                            <button type="button"
                                                    class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-primary hover:bg-primary-lighter hover:text-gray-100 focus:outline-none focus:bg-primary-dark focus:text-gray-100 active:bg-primary-dark active:text-gray-100 transition">
            ...
            <!-- Main footer -->
            <footer
                class="flex flex-shrink-0 items-center justify-between p-4 bg-white border-t dark:bg-darker dark:border-primary-darker"
            >
                <div>TOM bv &copy; {{ now()->year }}</div>
                <div>
                    Made by
...
<script>
    const setup = () => {
        ...
        const getCounterEnabled = () => {
            if (window.localStorage.getItem('counterEnabled')) {
                return JSON.parse(window.localStorage.getItem('counterEnabled'))
            }

            return false;
        }

        const setCounterEnabled = (value) => {
            window.localStorage.setItem('counterEnabled', value);
        }
        ...
        return {
            loading: true,
            isDark: getTheme(),
            counterEnabled: getCounterEnabled(),
            toggleCounter() {
